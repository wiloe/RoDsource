...
int get_trust( CHAR_DATA *ch )
{
	...
	/*BUG - NULL CHARACTER - spellsong for debugging purposes*/
	log_string( "BUG: NULL Ch pointer sent to get_trust in handler.c !" );
	return 0;
}
...
int get_curr_stat( CHAR_DATA *ch, int stat )
{
	...
	if (ch->race != race_lookup("lacerti")) 
		return URANGE(3,ch->perm_stat[stat] + ch->mod_stat[stat], max);
	else
	return URANGE(3,ch->perm_stat[stat] + ch->mod_stat[stat] + lacerti_stat(), max);
}

	/* Added by Divinos -- May 1st, 2000 */
int lacerti_stat(void)
{
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	/*trap check spellsong*/
	trap_enter( ch->in_room->contents, ch );
...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
	...
	case WEAR_EAR_L:	return     obj->value[type]; /*Start Spellsong ADD */
	case WEAR_EAR_R:	return     obj->value[type];
	case WEAR_FLOATING:	return     obj->value[type];
	case WEAR_BICEP_L:	return     obj->value[type];
	case WEAR_BICEP_R:	return     obj->value[type];
	case WEAR_FACE:	return     obj->value[type];
	case WEAR_ANKLE_L:	return     obj->value[type];
	case WEAR_ANKLE_R:	return     obj->value[type];
	case WEAR_SHOULDERS:	return 2 * obj->value[type];/*End Spellsong ADD*/
...
/***************FURNITURE kat 04171999*************************************/
/*Obj on objects functions*/
/*
 * Move an object onto an object.
 */
void obj_on_obj( OBJ_DATA *obj, OBJ_DATA *obj_to )
{
    obj->next_on		= obj_to->onlist; /*move ye ol' object down a slot*/
    obj_to->onlist 		= obj; /*Make current thing we're putting first in list*/
    obj->on_obj			= obj_to; /*Link the object to the object it's on*/
    obj->in_room		= NULL;
    obj->carried_by		= NULL;

    for ( ; obj_to != NULL; obj_to = obj_to->on_obj )
    {
	if ( obj_to->carried_by != NULL )
	{
	    obj_to->carried_by->carry_number += get_obj_number( obj );
...
/*
 * Move an object off of an object.
 */
void obj_off_obj( OBJ_DATA *obj )
{
    OBJ_DATA *obj_from;

    if ( ( obj_from = obj->on_obj ) == NULL )
    {
	bug( "Obj_off_obj: null obj_from.", 0 );
	return;
    }

    if ( obj == obj_from->onlist )
    {
	obj_from->onlist = obj->next_on;
    }
    else
    {
	OBJ_DATA *prev;

	for ( prev = obj_from->onlist; prev; prev = prev->next_on )
	{
	    if ( prev->next_on == obj )
	    {
		prev->next_on = obj->next_on;
		break;
	    }
	}
	...
	if ( prev == NULL )
	{
	    bug( "Obj_off_obj: obj not found.", 0 );
	    return;
	}
    }
    ...
    for ( ; obj_from != NULL; obj_from = obj_from->on_obj )
    {
...
/*End of object on stuff*/

/*Char at object stuff*/

void char_at_obj( CHAR_DATA *ch, OBJ_DATA *obj_to )
{
    ch->next_at	        	= obj_to->atlist; /*move ye ol' object down a slot*/
    obj_to->atlist 		= ch; /*Make current thing we're putting first in list*/
    ch->at_obj			= obj_to; /*Link the object to the object it's on*/
    /*Character remains in the room, even if he's at the furniture*/
    /*And we don't have to worry about the furniture adding weight to you :P*/
    return;
}
/*
 * Move an object off of an object.
 */
void char_off_obj( CHAR_DATA *ch )
{
    OBJ_DATA *obj_from;

    if ( ( obj_from = ch->at_obj ) == NULL ) /*hey.. this guy isn't at an object??*/
    {
	bug( "char_off_obj: null obj_from.", 0 );
	return;
    }

    if ( ch == obj_from->atlist ) /*Mke sure that bugger isn't the first in the list*/
    {
	obj_from->atlist = ch->next_at;
    }
    else
    {
	CHAR_DATA *prev;
	/*Start at the begining, go all the way down the list*/
	for ( prev = obj_from->atlist; prev; prev = prev->next_at )
	{
	    if ( prev->next_at == ch )
	    {
		prev->next_at = ch->next_at;
		break;
	    }

	if ( prev == NULL )
	{
	    bug( "ch_off_obj: ch not found.", 0 );
	    return;
	}
    }
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    if ( !IS_NPC(ch) && ( ch->pcdata->nemesis == NULL || IS_NPC(ch->pcdata->nemesis)) )
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_PROTECT_EVIL  ) strcat( buf, " protect evil"  );
    if ( vector & AFF_PROTECT_GOOD  ) strcat( buf, " protect good"  );
   /* if ( vector & AFF_PARALYSIS     ) strcat( buf, " paralysis"     ); Eris 30 April 2000 */
    ...
    if ( vector & AFF_SEVERED	    ) strcat( buf, " severed"       );
    if ( vector & AFF_PRESERVATION  ) strcat( buf, " preservation"  ); /*Eris 30 April 2000*/
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_GOOD         ) strcat( buf, " good"         );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *wear_bit_name(int wear_flags)
{
	...
	if (wear_flags & ITEM_WEAR_FLOATING		) strcat(buf, " floating");
	if (wear_flags & ITEM_WEAR_BICEP		) strcat(buf, " bicep");
...
char *weapon_bit_name(int weapon_flags)
{
	...
	if (weapon_flags & WEAPON_SERRATED ) strcat(buf, " serrated");
...
RAFFECT_DATA *new_raffect (char *type)
{
   RAFFECT_DATA *raf;
   sh_int itype;
   
   if (!is_number(type)) return NULL;  /* type must be numeric for now */
   itype = atoi(type);		       /* convert to numeric form */
   if (itype != 1) return NULL;	       /* type must be 1 for now */
   raf = alloc_perm(sizeof(*raf));     /* allocate memory for the new affect */
				       /* start setting default values (type 1) */
   raf->type = itype;		         /* obvious */
   raf->dam_name = str_dup("fire");      /* need lookup table for next three!!! */
   raf->room_message = str_dup("$n is burned by the fire in this room!");
   raf->vict_message = str_dup("You are burned by the fire in this room!");
   raf->timer = 1;		         /* not used in type 1 */
   raf->val0 = 10;		         /* minimum hp damage in type 1 */
   raf->val1 = 20;		         /* maximum hp damage in type 1 */
   raf->val2 = 0;		         /* not used in type 1 */
   return raf;			       /* return the structure */
}
